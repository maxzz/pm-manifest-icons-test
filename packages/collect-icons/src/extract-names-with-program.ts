...
export async function collectNamesWithProgram(rootDir: string, recursive: boolean, prefixes: string[]): Promise<Map<string, string[]>> {
    ...
    if (tsconfigPath) {
        ...
        if (!read.error) {
            ...
            const cfgFiles = parsed.fileNames && parsed.fileNames.length > 0 ? parsed.fileNames : filenames;
    ...
    for (const sf of program.getSourceFiles()) {
        ...
        if (!nf.startsWith(normalizedRoot)) {
            continue;
        }
